...
//		return fill += 16;
	...
	public long readLongFromHigh(int len) throws IOException {
		long result = 0L;

		// Ensure there are enough bits in the buffer or refill it.
		if (fill < 16) {
			refill();
		}

		// Check if the request can be satisfied with the current buffer content.
		if (len <= fill) {
			result = (current >>> (fill - len)) & ((1L << len) - 1);
			fill -= len;
			readBits += len;
			return result;
		}

		// More bits needed than currently available in 'fill'
		len -= fill;
		result = (current & ((1L << fill) - 1)) << len;  // Move current bits to the top
		readBits += fill;
		fill = 0;

		// Read the rest of the bits required.
		while (len >= 8) {
			int nextByte = read();
			result |= (long) nextByte << (len - 8);
			len -= 8;
			readBits += 8;
		}
