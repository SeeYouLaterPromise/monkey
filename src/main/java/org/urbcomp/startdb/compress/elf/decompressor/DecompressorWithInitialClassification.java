...
public class DecompressorWithInitialClassification {
//    private boolean deltaEQ0 = false;
    ...
    private boolean inferredIndex = false;
    private boolean nonErase = false;
    private final InputBitStream in;
    private final static long END_SIGN = Double.doubleToLongBits(Double.NaN);
    private final static short[] leadingRepresentation = {0, 8, 12, 16, 18, 20, 22, 24};

    public DecompressorWithInitialClassification(byte[] bs) {
...
//       2. process high precision case subsequently
        if (nonErase) {
...
//        4. read the left Mantissa, concatenating with lead
    ...
    private void nextValue() throws IOException {
        ...
        if (in.readBit() == 0) {
            if (in.readBit() == 0) {
                // identical case 00!  Alpha is like before as well.
                if (lastBetaStar == 0) negative = ((storedVal >>> 63) & ((1L << 1) - 1)) == 1;
                else inferredIndex = true;
                return;
            }else {
...
//        Best Case: PreValue could infer
        if (in.readBit() == 1) {
            // without the use of theta, so when recovering need to calculate Alpha
            inferredIndex = true;
            if (64 - PreValueTrailCount <= read) return;
            storedVal ^= in.readLong(64 - read - PreValueTrailCount) << PreValueTrailCount;
            return;
        }

//        Attention that for non-perform erasing case, there are two kinds of cases, order is essential.
        if (nonErase) {
