...
public class DecompressorWithThetaReturned {
    ...
    public DecompressorWithThetaReturned(byte[] bs) {
     ...
     */
    public Double readValue(int beta, int[] theta) {
        try {
            next(beta, theta);
        } catch (IOException e) {
    ...
    private void next(int beta, int[] theta) throws IOException {
        if (first) {
            firstValue(beta, theta);
        } else {
            nextValue(beta, theta);
        }
...
//            END
    ...
    private void firstValue(int beta, int[] theta) throws IOException {
        first = false;
...
//        store the calculation result, pay attention!!!
        ...
        if(mon64.furtherCheck(E)) {
            ...
            if (Integer_ >= mon64.Pow10(storedTheta)) {
                theta[0] = theta[0] + 1;
                beta--;
            }

            alpha = beta - storedTheta;
...
//        int ValueTrailCount = deltaEQ0 ? 52 - (alpha + E) : 52 - (mon64.getFAlpha(alpha) + E);
    ...
    private void nextValue(int beta, int[] theta) throws IOException {
...
//        PreValue couldn't infer
        ...
        if (E > 0 && read >= 12 + E) {
            ...
            if (Math.abs(v_l) >= mon64.Pow10(storedTheta)) {
                theta[0] = theta[0] + 1;
                beta--;
            }
            alpha = beta - storedTheta;
...
//        PreValueTrailCount = deltaEQ0 ? 52 - (alpha + E) : 52 - (mon64.getFAlpha(alpha) + E);
